...
function! s:chmod(...) abort
  ...
  if empty(l:modes)
    throw 's:chmod: Missing filemode specification.'
  endif

  if empty(l:files)
    throw 's:chmod: Missing filename.'
  endif
  ...
  for l:mode in split(l:modes[0], ',')
    ...
    elseif l:operator ==# '='
      ...
      throw printf("s:chmod(): An unknown operator '%s'", l:operator)
    endif
  endfor
  ...
  for l:file in l:files
    ...
    if l:verbose
      echo printf("mode of '%s' retained as %d (%s)", l:file, l:octal_mode, l:mode_new)
    endif
  endfor
endfunction
...
" :Chmod -v +x %
" :Chmod --verbose +x %
command! -bar -nargs=+ -complete=file Chmod call s:chmod(<f-args>)

function! s:make_executable(bang, ...) abort
  let l:sbang = get(a:000, 0, '') ==# '!'
  let l:mode = empty(a:bang) ? 'u+x'
    \ : l:sbang ? 'a+x'
    \ : 'ug+x'
  let l:files = l:sbang ? a:000[1:] : a:000

  call s:chmod([l:mode] + l:files)
endfunction

" A shorthand for ":Chmod" to make files executable.
command! -bar -bang -nargs=+ -complete=file MakeExecutable call s:make_executable(<q-bang>, <f-args>)
